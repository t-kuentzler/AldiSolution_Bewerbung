    ...
    public string SaveFileOnServer(byte[] content)
    {
        ...
        catch (UnauthorizedAccessException ex)
        {
            _logger.LogError(ex, "Zugriff verweigert beim Speichern der Datei im Verzeichnis {UploadsFolder}",
                uploadsFolder);
            throw new FileSaveException($"Zugriff verweigert beim Speichern der Datei im Verzeichnis {uploadsFolder}.",
                ex);
        }
        catch (Exception ex)
        {
    ...
    public MemoryStream GeneratePdf(Return? returnObj)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                boldFont = new XFont("Arial_bold", 12, XFontStyleEx.Bold);
            }
            catch (Exception)
            {
                _logger.LogError("Fehler beim Laden der Schriftarten.");
                throw;
            }
        ...
        {
            _logger.LogError(ex, "Fehler bei PDF-Generierung.");
