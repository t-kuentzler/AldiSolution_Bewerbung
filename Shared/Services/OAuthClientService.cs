...
public class OAuthClientService : IOAuthClientService
{
    ...
    public async Task<OrderResponse> GetApiOrdersAsync()
    {
        ...
        using (var client = _clientFactory.CreateClient())
        {
            ...
            string ordersUrl =
                $"{_settings.BaseUrl}aldivendorwebservices/2.0/DE/vendor/magmaheimtex_DE/orders?pageNumber=0&pageSize=100";
    ...
    public async Task<ReturnResponse> GetApiReturnsWithStatusCreatedAsync(string status)
    {
        ...
        try
        {
            using (var client = _clientFactory.CreateClient())
            {
                ...
                string returnUrl =
                    $"{_settings.BaseUrl}aldivendorwebservices/2.0/DE/vendor/{_settings.VendorId}/returnrequests";
        ...
        {
            _logger.LogError(ex, "HTTP-Anfragefehler bei der Kommunikation mit der API.");
            throw new ApiException("HTTP-Anfragefehler bei der Kommunikation mit der API, Message: " + ex.Message, ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unerwarteter Fehler bei der Kommunikation mit der API.");
            throw new ApiException("Unerwarteter Fehler bei der Kommunikation mit der API, Message: " + ex.Message, ex);
        }
    }
    ...
    public async Task<bool> ReportConsignmentDeliveryAsync(Consignment consignment, int retryCount = 0)
    {
        const int maxRetryCount = 5;
        const int delayDuration = 3000;

        _logger.LogInformation($"Meldung der Zustellung für Consignment mit Consignment Id {consignment.Id}.");
        ...
        using (var client = _clientFactory.CreateClient())
        {
            ...
            var deliveryReport = new
            {
                carrier = consignment.Carrier,
                entries = consignment.ConsignmentEntries.Select(e => new
                {
                    orderEntryNumber = e.OrderEntryNumber,
                    quantity = e.Quantity
                }),
                receiptDelivery = DateTime.Now.ToString("yyyy-MM-dd"),
                shippingAddress = new
                {
                    countryIsoCode = consignment.ShippingAddress.CountryIsoCode,
                    type = consignment.ShippingAddress.Type
            ...
            var jsonRequest = JsonConvert.SerializeObject(deliveryReport);
            var content = new StringContent(jsonRequest, Encoding.UTF8, "application/json");
            ...
            {
                _logger.LogInformation(
                    $"Zustellung für Consignment mit Consignment Id '{consignment.Id}' erfolgreich der API gemeldet.");
                return true;
            }
            else if (response.StatusCode == System.Net.HttpStatusCode.Unauthorized && retryCount < maxRetryCount)
            {
                _logger.LogWarning(
                    "Der Token ist ungültig. Es wird versucht, ihn zu aktualisieren und die Anfrage zu wiederholen.");
                await _accessTokenService.GetAndUpdateNewAccessToken();
                return await ReportConsignmentDeliveryAsync(consignment, retryCount + 1);
            }
            else
            {
                _logger.LogError(
                    $"Fehler bei der Meldung der Zustellung für Consignment {consignment.VendorConsignmentCode}. Status Code: {response.StatusCode}, Response: {await response.Content.ReadAsStringAsync()}");
                return false;
            }
        }
    }
