...
public class AccessTokenService : IAccessTokenService
    {
        ...
        public AccessTokenService(IAccessTokenRepository tokenRepository, IOAuthClientServiceFactory oAuthClientServiceFactory,
        ...
        private async Task ValidateAndSaveTokenAsync(AccessToken token)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new AccessTokenServiceException("Fehler bei der Erstellung des Initial-Datensatzes für den AccessToken.", ex);
            }
        }
        
        public async Task<(bool IsValid, AccessToken? Token)> IsTokenValid()
        {
            try
            {
                var token = await _tokenRepository.GetFirstTokenAsync();
        ...
        public async Task<string> GetAndUpdateNewAccessToken()
        {
            ...
            {
                throw new Exception("Es ist ein unerwarteter Fehler beim Abrufen des AccessToken von der API aufgetreten.", ex);
            }
        ...
        private async Task UpdateToken(string? newToken)
        {
            ...
            {
                _logger.LogError(ex, "Es ist ein unerwarteter Fehler beim aktualisieren des AccessToken in der Datenbank aufgetreten.");
        ...
        private async Task CreateNewToken(string newToken)
        {
            ...
            _logger.LogInformation("Es wird versucht einen neuen Datensatz für den AccessToken in der Datenbank zu erstellen.");
            ...
            {
                _logger.LogError(ex, "Es ist ein unerwarteter Fehler beim erstellen des AccessToken in der Datenbank aufgetreten.");
                throw new AccessTokenServiceException(
